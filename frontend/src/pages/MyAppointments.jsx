...
const MyAppointments = () => {
  ...
  const cancelAppointment = async (appointmentId) => {
    try {
      ...
      // here check userId for cancel booked appointment
  ...
  const initPay = (order) => {
    ...
    const options = {
      ...
      description:"Apppintment Payment",
      order_id:order.id,
  ...
  return (
    <div>
      <p className="pb-3 mt-12 text-2xl font-bold text-zinc-700 border-bold">
        My Appointments
      </p>
      <div className="text-base">
        {appointments.map((item, index) => (
          <div
            className="grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border border-gray-300"
            key={index}
          >
            <div>
            ...
            <div className="flex-1 text-sm text-zinc-600">
              ...
              <p className="text-zinc-800 font-bold mt-1">Address:</p>
              <p className="text-xs">{item.docData.address.line1}</p>
              <p className="text-xs">{item.docData.address.line2}</p>
              <p className="text-xs mt-1"><span className="text-sm text-neutral-700 font-bold">Date & Time: </span> {slotDateFormat(item.slotDate)} | {item.slotTime}</p>
            </div>
            <div></div>
